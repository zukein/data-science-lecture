...
# しかし、母分布の形にかかわらず、**サンプル数が多い**ときの**標本和・標本平均**の分布は正規分布に近づくことがわかっている。これを中心極限定理という。 *(証明にはモーメント母関数とテイラー展開を用いる)*
# サンプル数については**少なくとも 30 以上とされることが多い**。
#%% [markdown]
# ###### 練習問題
#
...
#%% [markdown]
# サンプルから母分散を推定する場合には、サンプル数 $n$ ではなく**自由度 $n-1$** で割らないと期待値 (同じ操作を繰り返して平均をとったときに収束していく値) が母分散よりも小さくなってしまう。このように推定量の期待値が母数と一致する性質を不偏性という。
# $n-1$ で割った $
# {\displaystyle
#     s^{2} =\frac
