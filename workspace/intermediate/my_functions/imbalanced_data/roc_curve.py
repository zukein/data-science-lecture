...
def plot(performance, threshold):
    ...
    tt = beta.ppf(np.linspace(0, 1, 500), a, b)
    ff = 1 - tt
    yy = np.hstack([np.ones_like(tt), np.zeros_like(ff)])
    yy_pred = np.hstack([tt, ff])
    fpr, tpr, _ = roc_curve(yy, yy_pred)
    ax2.plot(fpr, tpr, color='black', linewidth=0.5)
    ax2.scatter(beta.cdf(1 - threshold, a, b), beta.sf(threshold, a, b))
    ax2.set(title='ROC曲線',
            xlabel='偽陽性率',
            ylabel='真陽性率',
            xlim=(0, 1),
